...
const NftCreate: NextPage = () => {
  ...
  const handleImage = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setNftMeta({
        ...nftMeta,
        image: `${process.env.NEXT_PUBLIC_PINATA_DOMAIN}/ipfs/${data.IpfsHash}?pinataGatewayToken=${process.env.PINATA_GATEWAY_TOKEN}`
      });
    } catch(e: any) {
  ...
  const uploadMetadata = async () => {
    try {
      ...
      setNftURI(`${process.env.NEXT_PUBLIC_PINATA_DOMAIN}/ipfs/${data.IpfsHash}?pinataGatewayToken=${process.env.PINATA_GATEWAY_TOKEN}`);
    } catch (e: any) {
      console.error(e.message);
    }
  }
  ...
  const createNft = async () => {
    try {
      const nftRes = await axios.get(nftURI, {headers: {"Accept": "text/plain"}});
      const content = nftRes.data;

      // console.log("ttt", content, nftURI);
